...
//structure of parameter of zdp send confirm, luoyiming 2019-07-24
typedef struct
{
  /* task id */
  uint8_t srcServiceTaskId;
  /* pre message pointer */
  ...
  /** cnfParam of confirm callback,added by Luoyiming **/
  void *cnfParam;
}zdpCnfParam_t;
...
#ifndef ZNP_NPI
//set zdo data confirm, get transID and seqNum;
static zdpCnfParam_t* setZdoExtParam( uint8_t srcServiceTaskId, zstack_zdoExtParam_t* pZdoExtParam );

//clear zdo data confirm
static void clearZdoExtParam( zdpCnfParam_t* zdpCnfParam );

//confirm callback
static void zdpSendComfirmCallback( uint8_t status, uint8_t endpoint, uint8_t transID, uint16_t clusterID, void* cnfParam );
...
static void processAfDataConfirm( afDataConfirm_t *pkt )
{
  ...
  if ( pReq == NULL )
  {
    // Run af data confirm callback before ignore the message
    if( pkt->afCnfCB )
    {
      pkt->afCnfCB( pkt->hdr.status, pkt->endpoint, pkt->transID, pkt->clusterID, pkt->cnfParam );
    }
    // Ignore the message
    return;
  }
  ...
  //set confirm callback,added by Luoyiming
  pReq->req.afCnfCB = pkt->afCnfCB;
  pReq->req.cnfParam = pkt->cnfParam;
...
static void processAfReflectErrorInd( afReflectError_t *pkt )
{
  ...
  pReq->req.dstAddr.endpoint = pkt->dstEP;
...
static void zsProcessZDOMsgs( zdoIncomingMsg_t *inMsg )
{
  switch ( inMsg->clusterID )
  {
    ...
    case Bind_rsp:
      {
        uint8_t result[2];
        result[0] = inMsg->TransSeq;
        result[1] = ZDO_ParseBindRsp( inMsg );
    ...
    case Mgmt_Leave_rsp:
      {
        uint8_t result[2];
        result[0] = inMsg->TransSeq;
        result[1] = ZDO_ParseMgmtLeaveRsp( inMsg );
    ...
    case Mgmt_Direct_Join_rsp:
      {
        uint8_t result[2];
        result[0] = inMsg->TransSeq;
        result[1] = ZDO_ParseMgmtDirectJoinRsp( inMsg );
    ...
    case Mgmt_Permit_Join_rsp:
      {
        uint8_t result[2];
        result[0] = inMsg->TransSeq;
        result[1] = ZDO_ParseMgmtPermitJoinRsp( inMsg );
...
static void sendNwkAddrRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pNwkAddrRsp->rsp.transSeq = pAddrRsp->transSeq;  //add by luoyiming
...
static void sendIeeeAddrRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pIeeeAddrRsp->rsp.transSeq = pAddrRsp->transSeq;  //add by luoyiming
...
static void sendNodeDescRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.transSeq = pNdRsp->transSeq;  //add by luoyiming
  ...
  pRsp->rsp.nodeDesc.stackRev = pNdRsp->nodeDesc.ServerMask >> STACK_COMPLIANCE_CURRENT_REV_POS; // Add stack Rev in OS message, fixed by luoyiming 2019-12-02
...
static void sendPowerDescRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = pPowerRsp->transSeq;  //add by luoyiming
...
static void sendSimpleDescRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = pSimpleRsp->transSeq;  //add by luoyiming
...
static void sendActiveEPRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = pActiveEPRsp->transSeq;  //add by luoyiming
...
static void sendUserDescRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = pUdRsp->transSeq;  //add by luoyiming
...
static void sendServerDiscRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = pSdRsp->transSeq;  //add by luoyiming
...
static void sendBindRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  uint8_t *result = (uint8_t *)pMsg;  //fix by luoyiming
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = result[0];  //add by luoyiming
  pRsp->rsp.status = (zstack_ZdpStatus)result[1];
...
static void sendMgmtNwkDiscRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = pNwkDiscRsp->transSeq;  //add by luoyiming
...
static void sendMgmtLqiRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = pLqiRsp->transSeq;  //add by luoyiming
...
static void sendMgmtRtgRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = pRtgRsp->transSeq;  //add by luoyiming
...
static void sendMgmtBindRspInd( uint16_t dstID, uint16_t srcAddr, void *pMsg )
{
  ...
  pRsp->rsp.srcAddr = srcAddr;
  pRsp->rsp.transSeq = pBindRsp->transSeq;  //add by luoyiming
...
static bool processSecNwkKeySetReq( uint8_t srcServiceTaskId, void *pMsg )
{
  ...
  if ( pPtr->pReq )
  {
    ...
    if ( pPtr->pReq->has_key )
    {
      ...
      if( TRUE == OsalPort_isBufSet( pPtr->pReq->key, 0, 16 ) ) //if setting key is invalid key, reset NWK-Key. add by luoyiming 2019-08-22
      {
        ZDApp_ResetNwkKey();
      }
    }
  ...
  if ( pPtr->pReq )
  {
    ...
    else
    {
      ...
      if ( pEPDesc )
      {
        ...
        {
          pPtr->hdr.status = (zstack_ZStatusValues)AF_DataRequestExt( &dstAddr,
...
static bool processZdoNwkAddrReq( uint8_t srcServiceTaskId, void *pMsg )
{
  ...
  if ( pPtr->pReq )
  {
    ...
    //set ext param, luoyiming fix at 2019-07-24
    ...
    if( zdpCnfParam )
    {
      pPtr->hdr.status = (zstack_ZStatusValues)ZDP_NwkAddrReqExt(
            pPtr->pReq->ieeeAddr, pPtr->pReq->type, pPtr->pReq->startIndex,
  ...
  if ( pPtr->pReq )
  {
    ...
    if( zdpCnfParam )
    {
      pPtr->hdr.status = (zstack_ZStatusValues)ZDP_IEEEAddrReqExt(
            pPtr->pReq->nwkAddr, pPtr->pReq->type, pPtr->pReq->startIndex,
            FALSE, zdpSendComfirmCallback, zdpCnfParam ); // fixed by luoyiming 2021-04-26
      if( pPtr->hdr.status != zstack_ZStatusValues_ZSuccess )
      {
        //clear send param if send not success, fix at 2019-07-24
  ...
  if ( pPtr->pReq )
  {
    ...
    if( zdpCnfParam )
    {
      pPtr->hdr.status = (zstack_ZStatusValues)ZDP_MatchDescReqExt( &dstAddr,
            pPtr->pReq->nwkAddrOfInterest,
            pPtr->pReq->profileID,
            pPtr->pReq->n_inputClusters, pPtr->pReq->pInputClusters,
            pPtr->pReq->n_outputClusters, pPtr->pReq->pOutputClusters,
            FALSE, zdpSendComfirmCallback, zdpCnfParam );  // fixed by luoyiming 2021-04-26
      if( pPtr->hdr.status != zstack_ZStatusValues_ZSuccess )
      {
        //clear send param if send not success, fix at 2019-07-24
...
static bool processZdoServerDiscReq( uint8_t srcServiceTaskId, void *pMsg )
{
  ...
  if ( pPtr->pReq )
  {
    ...
    if( zdpCnfParam )
    {
      pPtr->hdr.status = (zstack_ZStatusValues)ZDP_ServerDiscReqExt(
            serverMask, FALSE, zdpSendComfirmCallback, zdpCnfParam ); // fixed by luoyiming 2021-04-26
      if( pPtr->hdr.status != zstack_ZStatusValues_ZSuccess )
      {
        //clear send param if send not success, fix at 2019-07-24
        clearZdoExtParam( zdpCnfParam );
...
static bool processZdoBindReq( uint8_t srcServiceTaskId, void *pMsg )
{
  ...
  if ( pPtr->pReq )
  {
    ...
    if ( srcBindAddr.addr.shortAddr == _NIB.nwkDevAddress )
    {
      ...
      {
        pPtr->hdr.status = zstack_ZStatusValues_ZSuccess;
        //if no ZDP_BindUnbindReq, data confirm callback is invalid. Add by Luoyiming.
        pPtr->pReq->extParam.afCnfCB = NULL;
        pPtr->pReq->extParam.cnfParam = NULL;
      }
      else
      {
        pPtr->hdr.status = zstack_ZStatusValues_ZInvalidParameter;
  ...
  if ( pPtr->pReq )
  {
    ...
    if( zdpCnfParam )
    {
      pPtr->hdr.status = (zstack_ZStatusValues)ZDP_MgmtNwkDiscReqExt(
            &dstAddr, pPtr->pReq->scanChannels, pPtr->pReq->scanDuration, pPtr->pReq->startIndex,
            FALSE, zdpSendComfirmCallback, zdpCnfParam ); // fixed by luoyiming 2021-04-26
      if( pPtr->hdr.status != zstack_ZStatusValues_ZSuccess )
      {
        //clear send param if send not success, fix at 2019-07-24
  ...
  if ( pPtr->pReq )
  {
    ...
    if( zdpCnfParam )
    {
      pPtr->hdr.status = (zstack_ZStatusValues)ZDP_MgmtLqiReqExt(
            &dstAddr, pPtr->pReq->startIndex, FALSE, zdpSendComfirmCallback, zdpCnfParam ); // fixed by luoyiming 2021-04-26
      if( pPtr->hdr.status != zstack_ZStatusValues_ZSuccess )
      {
        //clear send param if send not success, fix at 2019-07-24
        clearZdoExtParam( zdpCnfParam );
  ...
  if ( pPtr->pReq )
  {
    ...
    if( zdpCnfParam )
    {
      pPtr->hdr.status = (zstack_ZStatusValues)ZDP_MgmtDirectJoinReqExt( &dstAddr,
            pPtr->pReq->deviceAddress, cInfo, FALSE, zdpSendComfirmCallback, zdpCnfParam ); // fixed by luoyiming 2021-04-26
      if( pPtr->hdr.status != zstack_ZStatusValues_ZSuccess )
      {
        //clear send param if send not success, fix at 2019-07-24
        clearZdoExtParam( zdpCnfParam );
  ...
  if ( pPtr->pReq )
  {
    ...
    if( zdpCnfParam )
    {
      pPtr->hdr.status = (zstack_ZStatusValues)ZDP_DeviceAnnceEx(
            pPtr->pReq->nwkAddr, pPtr->pReq->ieeeAddr, cInfo, FALSE, zdpSendComfirmCallback, zdpCnfParam ); // fixed by luoyiming 2021-04-26
      if( pPtr->hdr.status != zstack_ZStatusValues_ZSuccess )
      {
        //clear send param if send not success, fix at 2019-07-24
        clearZdoExtParam( zdpCnfParam );
...
static bool processBdbSetAttributesReq( uint8_t srcServiceTaskId, void *pMsg )
{
  ...
  if ( pPtr->pReq )
  {
    ...
    {
    	bdb_setJoinUsesInstallCodeKey( pPtr->pReq->bdbJoinUsesInstallCodeKey );//install code setting valid, fix by luoyiming.
    }
...
static bool processBdbRepAddAttrCfgRecordDefaultToListReq( uint8_t srcServiceTaskId, void *pMsg )
{
  ...
  if ( pPtr->pReq )
  {
    pPtr->hdr.status = bdb_RepAddAttrCfgRecordDefaultToList(pPtr->pReq->endpoint,
                                         pPtr->pReq->cluster,
                                         pPtr->pReq->manuCode, //added by luoyiming, 2019-10-21
                                         pPtr->pReq->direction, //added by luoyiming, 2020-01-07
                                         pPtr->pReq->attrID,
...
static bool processBdbRepChangedAttrValueReq( uint8_t srcServiceTaskId, void *pMsg )
{
  ...
  if ( pPtr->pReq )
  {
    pPtr->hdr.status = bdb_RepChangedAttrValue(pPtr->pReq->endpoint,pPtr->pReq->cluster, pPtr->pReq->manuCode, pPtr->pReq->direction, pPtr->pReq->attrID); // fixed by luoyiming, 2020-01-07
  }
  else
  {
    pPtr->hdr.status = zstack_ZStatusValues_ZInvalidParameter;
 ...
 * @fn          setZdoExtParam
 *
 * @brief       Set Zdp external parameter,add by luoyiming,fix at 2019-07-24
 *
 * @param       srcServiceTaskId - Source Task ID
 *              pZdoExtParam - external parameter
 *
 * @return      pointer of zdp send parameter
 */
static zdpCnfParam_t* setZdoExtParam( uint8_t srcServiceTaskId, zstack_zdoExtParam_t* pZdoExtParam )
{
  zdpCnfParam_t *zdpCnfParam = MAP_osal_mem_alloc( sizeof(zdpCnfParam_t) );
  if( zdpCnfParam )
  {
    //pre-allocate message buffer for ZDP Send Confirm
    zdpCnfParam->pReq = (zstackmsg_afDataConfirmInd_t *)OsalPort_msgAllocate( sizeof(zstackmsg_afDataConfirmInd_t) );
    if( zdpCnfParam->pReq )
    {
      memset( zdpCnfParam->pReq, 0, sizeof(zstackmsg_afDataConfirmInd_t) );
      pZdoExtParam->transID = ZDP_GetTransID(); //get transID
      pZdoExtParam->seqNum = ZDP_SeqNum; // get ZDP Seq Number
 ...
 * @fn          clearZdoExtParam
 *
 * @brief       clear Zdp external parameter,add by luoyiming,fix at 2019-07-24
 *
 * @param       zdpCnfParam - pointer of zdp send parameter
 *
 * @return      NONE
 */
static void clearZdoExtParam( zdpCnfParam_t* zdpCnfParam )
{
  if( zdpCnfParam )
  {
    if( zdpCnfParam->pReq )
    {
      OsalPort_msgDeallocate( (uint8_t*)zdpCnfParam->pReq );
    }
    OsalPort_free( zdpCnfParam );
  }
}
...
/**************************************************************************************************
 *
 * @fn          zdpSendComfirmCallback
 *
 * @brief       data confirm callback of zdp send, add by luoyiming, 2019-07-24
 *              fixed at 2019-10-11, add param "clusterID" for 3.30 sdk
 *
 * @param       pfnAfCnfCB type in AF.h
 *
 * @return      NONE
 */
void zdpSendComfirmCallback( uint8_t status, uint8_t endpoint, uint8_t transID, uint16_t clusterID, void* cnfParam )
{
  zdpCnfParam_t* zdpCnfParam = cnfParam;
  if( zdpCnfParam )
  {
    uint8_t srcServiceTaskId = zdpCnfParam->srcServiceTaskId;
    zstackmsg_afDataConfirmInd_t *pReq = zdpCnfParam->pReq;
    zstack_AfCnfCb_t* afCnfCB = zdpCnfParam->afCnfCB;
    void *cnfParam = zdpCnfParam->cnfParam;
    OsalPort_free( zdpCnfParam ); //free zdp send cnf-param
    //change zdo confirm message into AF_DATA_CONFIRM_IND
    if( pReq )
    {
      pReq->hdr.event = zstackmsg_CmdIDs_AF_DATA_CONFIRM_IND;
      pReq->hdr.status = zstack_ZStatusValues_ZSuccess;
      pReq->req.status = (zstack_ZStatusValues)status;
      pReq->req.endpoint = ZDO_EP;
      pReq->req.transID = transID;
      pReq->req.clusterID = clusterID;  //for sdk 3.30, luoyiming fixed at 2019-10-11
      pReq->req.afCnfCB = afCnfCB;
      pReq->req.cnfParam = cnfParam;
      OsalPort_msgSend( srcServiceTaskId, (uint8_t*)pReq );
    }
  }
}
#endif
