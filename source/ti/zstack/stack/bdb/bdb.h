...
#include "dgp_stub.h"
     ...
     (ZSTACK_DEVICE_BUILD == DEVICE_BUILD_ROUTER) ||                 \
 ...
 */
ZStatus_t bdb_RepAddAttrCfgRecordDefaultToList( uint8_t endpoint, uint16_t cluster, uint16_t manuCdoe, uint8_t direction, uint16_t attrID,
                                                uint16_t minReportInt, uint16_t maxReportInt, uint8_t* reportableChange );
 ...
 */
ZStatus_t bdb_RepChangedAttrValue(uint8_t endpoint, uint16_t cluster, uint16_t manuCode, uint8_t direction, uint16_t attrID); //newvalue must a a buffer of size 8
#endif
