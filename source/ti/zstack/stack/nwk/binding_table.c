...
#endif
#ifdef BDB_REPORTING
            // marking both sever attribute reporting and client attribute reporting, fixed by luoyiming 2020-01-08
            bdb_RepMarkHasBindingInEndpointClusterArray(srcEpInt, clusterIds[index], 0, ZCL_FRAME_CLIENT_SERVER_DIR, BDBREPORTING_FALSE, BDBREPORTING_TRUE); // server attribute reporting
            bdb_RepMarkHasBindingInEndpointClusterArray(srcEpInt, clusterIds[index], 0, ZCL_FRAME_SERVER_CLIENT_DIR, BDBREPORTING_FALSE, BDBREPORTING_TRUE); // client attribute reporting
            // marking manufacture attribute, fixed by luoyiming 2020-01-08
            if ( clusterIds[index] < 0xFC00 )
            {
              uint16_t manuCode = zcl_GetManuCode( clusterIds[index] );
              bdb_RepMarkHasBindingInEndpointClusterArray(srcEpInt, clusterIds[index], manuCode, ZCL_FRAME_CLIENT_SERVER_DIR, BDBREPORTING_FALSE, BDBREPORTING_TRUE); // server attribute reporting
              bdb_RepMarkHasBindingInEndpointClusterArray(srcEpInt, clusterIds[index], manuCode, ZCL_FRAME_SERVER_CLIENT_DIR, BDBREPORTING_FALSE, BDBREPORTING_TRUE); // client attribute reporting
            }
            bindAdded = TRUE;
#endif
          }
