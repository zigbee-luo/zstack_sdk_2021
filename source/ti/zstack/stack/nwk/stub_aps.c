...
ZStatus_t INTERP_DataReq( APSDE_DataReq_t *req )
{
  ...
  if ( channelChangeInProgress || !StubAPS_InterPan( req->dstPanId, req->dstEP ) || ( req->srcEP != appEndPoint ) )  //match srcEP, fixed by luoyiming 2019-11-17
    return ( ZFailure );
...
{
  afDataConfirm( appEndPoint, dataCnf->msduHandle, 0xFFFF, dataCnf->hdr.Status );
